...
class Edge(object):
    ...
    def getDistanceTailHeadMeters(self):
        if (isinstance(self._tail, (WayPoint, Airport)) and isinstance(
                self._head, (WayPoint, Airport))):
            self.distanceTailHeadMeters = self._tail.getDistanceMetersTo(
                self._head)
    ...
    def getBearingTailHeadDegrees(self):
        if (isinstance(self._tail, (WayPoint, Airport)) and isinstance(
                self._head, (WayPoint, Airport))):
            self.bearingTailHeadDegrees = self._tail.getBearingDegreesTo(
                self._head)
...
class Graph(object):
    ...
    def __str__(self):
        return self.className + ': number of vertices= {0}'.format(
            len(self._vertex))
    ...
    def addVertex(self, *args):
        if len(args) == 1:
            ...
            if (index >= 0) and (index <= len(self._vertex)):
                ...
                raise ValueError(
                    self.className + ': insert index= {0] not in the limits 0..len'.format(
    ...
    def getVertex(self, v):
        ...
        if v < 0 or v >= len(self._vertex):
            raise ValueError(
                self.className + ': getVertex: vertex index out of bounds!!!')
    ...
    def insertEdge(self, index, baseEdge):
        ...
        if (index >= 0) and (index <= len(self._edge)):
            if (isinstance(baseEdge, Edge)):
                ...
                raise ValueError(
                    'Graph: insert edge - edge must be of class BaseEdge !!!')
    ...
    def addEdge(self, baseEdge):
        ...
        if (isinstance(baseEdge, Edge)):
            ...
            raise ValueError(
                'Graph: add edge - edge must be of class BaseEdge !!!')
    ...
    def createKmlOutputFile(self):
        """
        This function export the position over time of aircraft (which has
        graph structure) to KML format
        ...
        if self.getNumberOfVertices() > 1:
            ...
            strFileName += '-{0}.kml'.format(
                datetime.now().strftime("%d-%b-%Y-%Hh%Mm%S"))
    ...
    def createXlsxOutputFile(self):
        if self.getNumberOfVertices() > 1:
            ...
            for vertex in self.getVertices():
                ...
                edge = None
                if index > 0:
                    edge = Edge(self.getVertex(index - 1).getWeight(),
                                self.getVertex(index).getWeight())

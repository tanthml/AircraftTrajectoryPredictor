...
@author: Tan TRAN
        ...
        Written By:
                Tan TRAN
                @Email: < tanthm1102 (--AT--) gmail [--DOT--] com >

        @tanthml
        @copyright: Copyright 2020 Tan TRAN
...
manage a flight path built from a simple flight plan only contains waypoints
there is no departure and arrival airport
...
class FlightPathSim(FlightPlan):
    flightPlan = None
    aircraftICAOcode = ''
    ...
    def __init__(self,
        ...
        if self.getDepartureAirport():
            departure_alt = self.getDepartureAirport().getFieldElevationAboveSeaLevelMeters()
    ...
    def setAircraftStateAtInitWaypoint(self):
    ...
    def turnAndFly(self,
        ...
        """
        print(
            '================== one Turn Leg {} for each fix in the list =============== '.format(
                headWayPoint
            ))
        ...
        finalHeadingDegrees = initialHeadingDegrees
        finalWayPoint = headWayPoint
    ...
    def computeFlight(self, deltaTimeSeconds=1.0):
        """
        main entry to compute a whole flight
        Args:
            deltaTimeSeconds (float): delta t between fly step
        ...
        headWayPoint = self.wayPointsDict[self.fixList[0]]
        initialWayPoint = self.wayPointsDict[self.fixList[1]]
        initialHeadingDegrees = headWayPoint.getBearingDegreesTo(
            initialWayPoint
        )
        # init first gc route, and add to finalRoute graph
        first_gc_route = GreatCircleRoute(
            initialWayPoint=headWayPoint,
            finalWayPoint=initialWayPoint,
            aircraft=self.aircraft
        )
        distanceStillToFlyMeters = headWayPoint.getDistanceMetersTo(initialWayPoint)
        first_gc_route.computeGreatCircle(
            deltaTimeSeconds=deltaTimeSeconds,
            elapsedTimeSeconds=0.0,
            distanceStillToFlyMeters=distanceStillToFlyMeters,
            distanceToLastFixMeters=distanceStillToFlyMeters
        )
        self.finalRoute = first_gc_route

        # init approach waypoint
        self.aircraft.setTargetApproachWayPoint(self.wayPointsDict[self.fixList[-1]])

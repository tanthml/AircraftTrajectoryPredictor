...
class FlightPath(FlightPlan):
    ...
    def __init__(self,
        ...
        self.aircraft = None
        self.getAircraft()
        assert isinstance(self.aircraft, BadaAircraft) and not (
                    self.aircraft is None)
        ...
        if self.arrivalAirport is None:
            print(
                self.className + ': there is no arrival airport => flight is out-bound !!!')
        ...
        assert isinstance(self.departureAirport, Airport) and not (
                    self.departureAirport is None)

    def getAircraft(self):

        print(
            self.className + ': ================ get aircraft =================')
        atmosphere = Atmosphere()
        earth = Earth()
        ...
        if (acBd.aircraftExists(self.aircraftICAOcode) and
            ...
            raise ValueError(
                self.className + ': aircraft not found= ' + self.aircraftICAOcode)
    ...
    def turnAndFly(self,
        ...
        Args:
            tailWayPoint (WayPoint):
            headWayPoint (WayPoint):
            initialHeadingDegrees (float):
            headWayPointIndex (int):
        ...
        if (endOfSimulation == False):
            ...
            print(
                self.className + ': end of turn orientation= {0:.2f} degrees'.format(
            ...
            if (self.flightListIndex + 2) < len(self.fixList):
                ...
                angleDifferenceDegrees = math.degrees(
                    math.atan2(math.sin(secondAngleRadians - firstAngleRadians),
                               math.cos(
                                   secondAngleRadians - firstAngleRadians)))
                ...
                radiusOfTurnMeters = (
                                                 tasMetersPerSecond * tasMetersPerSecond) / (
                                                 9.81 * math.tan(
                                             math.radians(15.0)))
                ...
                print(
                    self.className + ': anticipated turn start from end point= {0:.2f} meters'.format(
                ...
                if ((endOfTurnLegWayPoint.getDistanceMetersTo(headWayPoint) > (
                    ...
                    print(
                        self.className + ': Envisage anticipated Fly By turn !!!!!!!!!!!!!!!!!!!!!!!!!')
            ...
            distanceStillToFlyMeters = self.flightLengthMeters - self.finalRoute.getLengthMeters()
            print(
                self.className + ': still to fly= {0:.2f} meters - still to fly= {1:.2f} nautics'.format(
    ...
    def loopThroughFixList(self, initialHeadingDegrees, elapsedTimeSeconds):
        ...
        while (endOfSimulation == False) and (
                self.flightListIndex < len(self.fixList)):
            ...
            print("fix, tailWayPoint: ", self.flightListIndex,
                  len(self.fixList), fix, tailWayPoint)
            ...
            if (self.flightListIndex + 1) < len(self.fixList):
                ...
                headWayPoint = self.wayPointsDict[
                    self.fixList[self.flightListIndex + 1]]
                print(headWayPoint)
                ''' turn and fly '''
    ...
    def buildDeparturePhase(self):
        ...
        print(self.className + ': last leg orientation= {0:.2f} degrees'.format(
            initialHeadingDegrees))
    ...
    def buildSimulatedArrivalPhase(self):
        ...
        print(
            self.className + '=========== add final turn, descent and ground run ===================')
        ...
        print(
            self.className + '===================== final 3 degrees descending glide slope ================')
        ...
        print(self.className + ': begin of last turn= {0}'.format(
            beginOfLastTurnLeg))
        ...
        print(
            self.className + ': updated flight path length= {0:.2f} nautics'.format(
    ...
    def buildArrivalPhase(self, initialHeadingDegrees):
        ...
        if self.isDomestic() or self.isInBound():
            ...
            print(
                self.className + ': runway final heading= {0:.2f} degrees'.format(
                    finalHeadingDegrees))
            ...
            print(
                self.className + ': distance still to fly= {0:.2f} nautics'.format(
                    distanceStillToFlyMeters * Meter2NauticalMiles))
            ...
            if str(fix).startswith('ADEP'):
                if index == 0:
                    if len(str(fix).split('/')) >= 2:
                        ...
                        departureAirport = airportsDb.getAirportFromICAOCode(
                            ICAOcode=departureAirportIcaoCode)
                    ...
                    if not (self.departureAirport is None):
                        departureRunway = runwaysDb.getFilteredRunWays(
                            airportICAOcode=departureAirportIcaoCode,
                            runwayName=departureRunwayName)
                else:
                    raise ValueError(
                        self.className + ': ADEP must be the first fix in the route!!!')
                ...
                if index == (len(strRoute.split('-')) - 1):
                    if len(str(fix).split('/')) >= 2:
                        ...
                        arrivalAirport = airportsDb.getAirportFromICAOCode(
                            ICAOcode=arrivalAirportIcaoCode)
                    arrivalRunwayName = ''
                    if len(str(fix).split('/')) >= 3:
                    ...
                    raise ValueError(
                        self.classeName + ': ADES must be the last fix of the route!!!')
    ...
    def computePairwiseDistances(self, fixes):
        ...
        for (prev, current) in pairwise(fixes):
            origin, destination = self.wayPointsDict[prev], self.wayPointsDict[
                current]
    ...
    def buildAircaft(self):
        """
        Build aircraft object via ICAO code with aircraft performance database
        Returns:
            (BadaAircraft):
        """
        if (aircraftDb.aircraftExists(self.aircraftICAOcode) and
                aircraftDb.aircraftPerformanceFileExists(
                    self.aircraftICAOcode)):
            aircraft = BadaAircraft(
                ICAOcode=self.aircraftICAOcode,
                aircraftFullName=aircraftDb.getAircraftFullName(
                    self.aircraftICAOcode),
            ...
            assert 15.0 <= self.RequestedFlightLevel <= 450.0

            aircraft.setTargetCruiseFlightLevel(
                RequestedFlightLevel=self.RequestedFlightLevel,
                departureAirportAltitudeMSLmeters=self.getDepartureAirport().getFieldElevationAboveSeaLevelMeters())
    ...
    def depart(self, aircraft, fixes, deltaTimeSeconds=1):
        ...
        distanceToLastFixMeters = self.departureAirport.getDistanceMetersTo(
            wayPoints[1]) + sum(distances[1:])
        estimatedFlightLength = self.departureAirport.getDistanceMetersTo(
            wayPoints[0]) + sum(distances) + wayPoints[
    ...
    def simulateArrival(self, aircraft, fixes, deltaTimeSeconds=1):
        ...
        distanceToLastFixNautics = touchDownWayPoint.getDistanceMetersTo(
            wayPoints[-1]) * Meter2NauticalMiles
        descentGlideSlope = DescentGlideSlope(runway=self.arrivalRunway,
                                              aircraft=aircraft,
                                              arrivalAirport=self.arrivalAirport,
                                              descentGlideSlopeDegrees=3.0)
    ...
    def flyFrom(self, aircraft, fixes, flightIndex, anticipatedTurnWayPoint,
                distanceStillToFlyMeters, lastLegEdge,
            ...
            if (endOfSimulation == False):
                ...
                if (flightIndex + 2) < len(fixes):
                    firstAngleDegrees = endOfTurnLegWayPoint.getBearingDegreesTo(
                        headWayPoint)
                    secondAngleDegrees = headWayPoint.getBearingDegreesTo(
                        self.wayPointsDict[fixes[flightIndex + 2]])
                    firstAngleRadians = math.radians(firstAngleDegrees)
                    secondAngleRadians = math.radians(secondAngleDegrees)

                    angleDifferenceDegrees = math.degrees(math.atan2(
                    ...
                    anticipatedTurnStartMeters = radiusOfTurnMeters * math.tan(
                        math.radians(
                            (180.0 - abs(angleDifferenceDegrees)) / 2.0))

                    if ((endOfTurnLegWayPoint.getDistanceMetersTo(
                            headWayPoint) > (1.1 * anticipatedTurnStartMeters)
                         and abs(angleDifferenceDegrees) > 30.)):
    ...
    def arrive(self, aircraft, initialHeadingDegrees,
               endOfLastGreatCircleWayPoint, touchDownWayPoint,
               distanceStillToFlyMeters, flownDistance):
        route = Graph()
    ...
    def simulateFly(self, startFrom, flightPath, updatedSpeed=None,
                    updatedAltitude=None):
        _, _, _, _, _, fixes, wayPointsDict = self.convertStrRouteToFixList(
            flightPath)
        ...
        with open('configurations.json', 'r') as f:
            ...
            finalRoute, initialWayPoint, initialHeadingDegrees = self.depart(
                departedAircraft, self.fixList,
                deltaTimeSeconds=1)
            beginOfLastTurnLeg, touchDownWayPoint, descentGlideSlopeLength = self.simulateArrival(
                departedAircraft,
                self.fixList)
            ...
            if configuration['previousVertex']:
                ...
                last = WayPoint(
                    ...
                    AltitudeMeanSeaLevelMeters=lastVertex[
            ...
            distances = self.computePairwiseDistances(fixes)
            flightLength = self.departureAirport.getDistanceMetersTo(
                wayPoints[0]) + sum(distances) + wayPoints[
                               -1].getDistanceMetersTo(
                self.arrivalAirport) + descentGlideSlopeLength
            distanceStillToFlyMeters = flightLength - configuration[
                'flownDistance']
            ...
            while (endOfSimulation == False) and (flightIndex < len(fixes) - 1):
                endOfSimulation, anticipatedTurnWayPoint, distanceStillToFlyMeters, last, elapsedTimeSeconds, initialHeadingDegrees, route = self.flyFrom(
                    aircraft, fixes, flightIndex, anticipatedTurnWayPoint,
                    distanceStillToFlyMeters, last,
                    elapsedTimeSeconds, initialHeadingDegrees
                )
            ...
            route = self.arrive(aircraft, initialHeadingDegrees,
                                fRoute.getLastVertex().getWeight(),
                                touchDownWayPoint, distanceStillToFlyMeters,
                                flightLength)
            fRoute.addGraph(route)
    ...
    def fly(self, deltaTimeSeconds=1):
        ...
        aircraft = self.buildAircaft()
        finalRoute, initialWayPoint, initialHeadingDegrees = self.depart(
            aircraft, fixes, deltaTimeSeconds)
        beginOfLastTurnLeg, touchDownWayPoint, descentGlideSlopeLength = self.simulateArrival(
            aircraft, fixes,
            deltaTimeSeconds)
        ...
        while (endOfSimulation == False) and (flightIndex < len(self.fixList)):
            ...
            endOfSimulation, anticipatedTurnWayPoint, distanceStillToFlyMeters, last, elapsedTimeSeconds, initialHeadingDegrees, _ = self.flyFrom(
                aircraft, self.fixList, flightIndex, anticipatedTurnWayPoint,
                distanceStillToFlyMeters, last,
                elapsedTimeSeconds, initialHeadingDegrees,
                deltaTimeSeconds=1
            )

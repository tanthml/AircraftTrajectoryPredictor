...
class TurnLeg(Graph):
    ...
    def buildTurnLeg(self,
        ...
        if ((2 * radiusOfTurnMeters) > self.initialWayPoint.getDistanceMetersTo(
                self.finalWayPoint)):
        ...
        print(
            self.className + ': tas= {0:.2f} knots - radius of turn= {1:.2f} meters - radius of turn= {2:.2f} nautics'.format(
                tasKnots, radiusOfTurnMeters,
                radiusOfTurnMeters * Meter2NauticalMiles))
        ...
        while ((endOfSimulation == False) and (continueTurning == True)):
            ...
            if self.stepDegrees > 0:
                ...
                if self.initialHeadingDegrees <= self.finalHeadingDegrees:
                    continueTurning = (
                                currentHeadingDegrees <= self.finalHeadingDegrees)
                else:
                    ...
                    if passedThrough360 == False:
                        ...
                        currentHeadingDegrees = math.fmod(currentHeadingDegrees,
                                                          360.0)
                        continueTurning = (
                                    currentHeadingDegrees <= self.finalHeadingDegrees)
        ...
        '''' print final heading  '''
        print(self.className + ': final heading= {0:.2f} degrees'.format(
            currentHeadingDegrees))
